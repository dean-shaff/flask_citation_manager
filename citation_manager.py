...
class Citation_Manager(object):
    ...
    def __init__(self,**kwargs):
        ...
        couch_cred = kwargs.get('couch_cred',None)
        if couch_cred:
            self.load_db(couch_cred)
        json_file = kwargs.get('json_file',None)    
        if json_file:
            self.load_json_file(json_file) 

    def load_db(self, couch_cred):
        """
        ...
        docs = [db[doc] for doc in db]
        self.entries = [Entry(**e) for e in docs]
    ...
    def create_json(self,save=False, filename=None):
        """
